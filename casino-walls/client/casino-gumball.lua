...
local inZone = false
local GumballSpawn = nil
local EvilPed = nil
local LastPedTurn = nil

-- 1. INTERACT WITH GUMBALL (Converted entirely to ox_lib Context)
local function InteractWithGumball()
    lib.hideTextUI()
    
    if Config.SecretGumball.Machine.Payment == 'items' then
        lib.registerContext({
            id = 'gumball_machine_menu',
            title = 'Gumball Machine',
            options = {
                {
                    title = 'Request Gumball',
                    description = 'Requires both membership cards & ' .. Config.SecretGumball.Machine.Price .. ' Casino Chips',
                    icon = 'candy-cane',
                    onSelect = function()
                        TriggerServerEvent('doj:server:checkItemsForGumball')
                    end
                }
            }
        })
        lib.showContext('gumball_machine_menu')
        
    elseif Config.SecretGumball.Machine.Payment == 'cash' then
        lib.registerContext({
            id = 'gumball_machine_menu_cash',
            title = 'Gumball Machine',
            options = {
                {
                    title = 'Request Gumball',
                    description = 'Price: $' .. Config.SecretGumball.Machine.Price .. ' cold cash',
                    icon = 'money-bill',
                    onSelect = function()
                        TriggerServerEvent('doj:server:checkItemsForGumball')
                    end
        ...
        lib.showContext('gumball_machine_menu_cash')
    end
end
...
-- 2. SPAWN LOOT CRATES (Converted qb-target to native ox_target)
local function spawnGumballLoot()
    CreateThread(function()
        for k, v in pairs(Config.SecretGumball.Loot.Spawn) do
            ...
            -- FIX: Converted old qb-target syntax into modern ox_target parameters
            exports.ox_target:addModel(prop, {
                {
                    name = 'gumball_loot_crate',
...
local function despawnGumballLoot()
    ...
    -- FIX: Replaced old clearing method with native ox_target local entity purge
    exports.ox_target:removeModel('xs_prop_arena_crate_01a', 'gumball_loot_crate')
    Crates = {}
end
...
RegisterNetEvent('doj:client:RefreshZones', function()
    if GumballZone then GumballZone:remove() end
    if NPCProp then
...
local function SecretLootLocation()
    local playerPed = cache.ped
...
function StartClonePed()
    ...
    -- Inline threat tracker loop to clean dependencies up
    CreateThread(function()
        while inZone and DoesEntityExist(EvilPed) do
            local dist = #(GetEntityCoords(cache.ped) - GetEntityCoords(EvilPed))
            if dist > 5.0 then
                TaskGoToEntity(EvilPed, cache.ped, -1, 4.0, 100.0, 1073741824, 0)
                Wait(1000)
            elseif not IsTaskMoveNetworkActive(EvilPed) then
...
function SpawnBackToCasino()
    ...
    if not IsPedRagdoll(playerPed) and IsPedOnFoot(playerPed) then
        ...
        DoScreenFadeOut(500)
        Wait(1000)
        if EvilPed then DeletePed(EvilPed) end
...
exports.ox_target:addModel('xs_prop_arena_crate_01a', {
    {
        name = 'gumball_loot_crate',
        event = 'doj:client:OpenGumballCrate',
        icon = 'fas fa-magnifying-glass',
